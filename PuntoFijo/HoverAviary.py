...
class HoverAviary(BaseRLAviary):
    ...
    def _computeTerminated(self):
        ...
        #     return False
        state = self._getDroneStateVector(0)
        if np.linalg.norm(self.TARGET_POS - state[0:3]) < 0.1:  # Si el dron alcanza el objetivo
            # Generar un nuevo objetivo aleatorio dentro del rango [0.01, 3] con 1 decimal
            self.TARGET_POS = np.round(np.random.uniform(low=0.01, high=3, size=(3,)), 1)
            print(f"[INFO] Nuevo objetivo generado: {self.TARGET_POS}")
            return False  # No termina el episodio, continúa hacia el nuevo objetivo
