...
class TestRechazoPert(BaseRLAviary):
    def __init__(self,
        ...
        if self.trajectory_type == "circle":
            self.WAYPOINTS = self._generate_smooth_circle_waypoints(self.CENTER, self.RADIUS, self.NUM_WAYPOINTS, smooth_points=10)
            ...
            self.WAYPOINTS = np.array([[1.63, 2.47, 1.14]])
        else:
        ...
        # Atributos para la perturbación
        self.perturbation_applied = False
        self.PERTURBATION_FORCE = np.array([0.0, 0.0, 0.5])  # Fuerza de perturbación (ejemplo: hacia arriba)
        self.PERTURBATION_DURATION_STEPS = int(self.CTRL_FREQ * 0.5) # Duración de la perturbación en pasos de control (ej: 0.5 segundos)
        self.perturbation_step_counter = 0
        self.PERTURBATION_TIME_SEC = 10.0 # Tiempo en segundos para aplicar la perturbación
        self.perturbation_trigger_step = int(self.PERTURBATION_TIME_SEC * self.PYB_FREQ) # Paso de simulación para la perturbación
    ...
    def reset(self, seed=None, options=None):
        ...
        self.perturbation_step_counter = 0 # Reiniciar el contador de pasos de perturbación
    ...
    def _computeTerminated(self):
        ...
        # --- NUEVO: Lógica para aplicar perturbación a los 15 segundos ---
        if not self.perturbation_applied and self.step_counter >= self.perturbation_trigger_step:
            print(f"[INFO] Aplicando perturbación al dron a los {self.PERTURBATION_TIME_SEC} segundos!")
            self.perturbation_applied = True
            p.applyExternalForce(self.DRONE_IDS[0], 
                                 -1, # -1 para el centro de masa del cuerpo base
                                 self.PERTURBATION_FORCE, 
                                 state[0:3], # Posición donde se aplica la fuerza
                                 p.WORLD_FRAME, # La fuerza se aplica en el sistema de coordenadas global
                                 physicsClientId=self.CLIENT)
        # --- FIN NUEVO ---

        # --- Lógica para mantener la perturbación por un tiempo si ya se aplicó ---
        if self.perturbation_applied:
            ...
            if self.perturbation_step_counter >= self.PERTURBATION_DURATION_STEPS:
                self.perturbation_applied = False # La perturbación ha terminado
                self.perturbation_step_counter = 0
        # --- FIN LÓGICA PERTURBACIÓN ---
        ...
        if pos_error < 0.1:
            ...
            if self.current_target_idx < len(self.WAYPOINTS) - 1:
                ...
                self.TARGET_POS = self.WAYPOINTS[self.current_target_idx]
            return False # No termina el episodio, sino que avanza al siguiente waypoint
