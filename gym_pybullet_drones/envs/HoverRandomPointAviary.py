...
class HoverRandomPointAviary(BaseRLAviary):
    ...
    def __init__(self,
        ...
        self.HOLD_STEPS    = int(4 * ctrl_freq)   # mantener 2 s en el radio
    ...
    def _computeTerminated(self):
        ...
        if dist < 0.1:
            ...
            if self._hold_counter >= self.HOLD_STEPS:
                print(f"[INFO] Objetivo alcanzado y estable 2 s: {self.TARGET_POS}")
                return True                       
        else:
            self._hold_counter = 0                # salió del radio → reset contador   
        
        if (abs(state[0]) > 3.5 or abs(state[1]) > 3.5 or state[2] > 3.5 or  # Condiciones de truncamiento
