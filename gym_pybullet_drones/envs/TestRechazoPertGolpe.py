...
class TestRechazoPertGolpe(BaseRLAviary):
    def __init__(self,
        ...
        # Atributos para la perturbación (simplificados para un golpe único)
        self.perturbation_applied = False
        # Ajusta la magnitud de esta fuerza para simular un "golpe".
        # Una fuerza mayor causará un efecto más notorio.
        self.PERTURBATION_FORCE_MAGNITUDE = np.array([0.0, 0.0, 6.5])  # Fuerza del golpe (ejemplo: hacia arriba)
        self.PERTURBATION_TIME_SEC = 10.0 # Tiempo en segundos para aplicar la perturbación
        self.perturbation_trigger_step = int(self.PERTURBATION_TIME_SEC * pyb_freq) # Usa pyb_freq del parámetro, no self.PYB_FREQ todavía
    ...
    def _computeTerminated(self):
        ...
        # --- Lógica para aplicar UN GOLPE (fuerza por un solo paso de simulación) a los 15 segundos ---
        if not self.perturbation_applied and self.step_counter >= self.perturbation_trigger_step:
            ...
            p.applyExternalForce(self.DRONE_IDS[0], 
                                 ...
                                 self.PERTURBATION_FORCE_MAGNITUDE, # Usamos la magnitud de fuerza para el golpe
